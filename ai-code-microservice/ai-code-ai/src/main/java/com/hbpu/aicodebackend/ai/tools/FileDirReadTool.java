...
public class FileDirReadTool extends BaseTool {
    ...
    public String readDir(
        ...
        try {
            String safeRelativeDirPath = StrUtil.nullToDefault(relativeDirPath, "");
            Path path = Paths.get(safeRelativeDirPath);
            if (!path.isAbsolute()) {
                String projectDirName = "vue_project_" + appId;
                Path projectRoot = Paths.get(AppConstant.CODE_OUTPUT_ROOT_DIR, projectDirName);
                path = projectRoot.resolve(safeRelativeDirPath);
            }
    ...
    public String generateToolExecutedResult(JSONObject arguments, String result) {
        String relativeDirPath = arguments == null ? null : arguments.getStr("relativeDirPath");
        String body = generateCodeBlock("text", normalize(result, ""));
        String title = String.format(
                "%s 读取目录：<code>%s</code>",
                resolveStatusIcon(result),
                normalize(relativeDirPath, "./")
        );
        return generateDetailsBlock("📂", title, body);
    }
}
