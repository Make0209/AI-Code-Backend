package com.hbpu.aicodebackend.langgraph4j;
...
import com.hbpu.aicodebackend.langgraph4j.node.concurrent.*;
...
public class CodeGenConcurrentWorkflow {
    ...
    public WorkflowContext executeWorkflow(String originalPrompt) {
        ...
        log.info("并发工作流图:\n{}", graph.content());
        // 1. 创建线程池（建议使用虚拟线程或固定线程池）
        ExecutorService executor = Executors.newVirtualThreadPerTaskExecutor();
        // 2. 配置 RunnableConfig
        // 注意：这里的 nodeId 应该是触发并发分支的那个节点 ID，即 "image_plan"
        RunnableConfig config = RunnableConfig.builder()
                                              .addParallelNodeExecutor("image_plan", executor)
                                              .build();
